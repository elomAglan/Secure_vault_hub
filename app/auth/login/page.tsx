...
export default function UserLoginPage() {
  ...
  const { login, isLoading, error: storeError, setError: setStoreError, clearError } = useAuthStore()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err?.response?.status === 401 || err?.message?.toLowerCase().includes('credential')) {
      ...
      // Optionnel : stocker dans le store
      setStoreError(err?.message || "Erreur inconnue")
    }
  }
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="w-full max-w-md">
            <Card className="border-slate-200/60 bg-white p-8 shadow-2xl shadow-slate-200/50 rounded-[2rem]">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between ml-1">
                    ...
                    <Link href="/auth/forgot-password" className="text-xs font-bold text-blue-600 hover:text-blue-700">Oublié ?</Link>
                  </div>
                  <div className="relative">
                ...
                {error && (
                  <motion.p
                    ...
                    className="text-sm text-red-500 text-center bg-red-50 p-3 rounded-xl border border-red-100 font-medium"
                  >
                    {error}
                  </motion.p>
                )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <Button variant="outline" type="button" asChild className="h-12 border-slate-200 hover:bg-slate-50 rounded-xl text-slate-600 gap-2 font-bold transition-all active:scale-95">
